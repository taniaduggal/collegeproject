...
if($paper_type=="minor1")
{
    ...
    where results.coursename='".$course."' && results.sem_id=".$sem." && results.subject_code='".$sub."' && results.minor_1_marks!=0 && admin_add_student.university_rollno='".$roll_no."' order by admin_add_student.student_id asc";
}
//echo $s;
...
else if($paper_type=="minor2")
{
    ...
    results.minor_2_marks from results inner join admin_add_student on admin_add_student.student_id=results.student_id
     where results.coursename='".$course."' && results.sem_id=".$sem." &&  results.subject_code='".$sub."' &&results.minor_2_marks!=0 &&  admin_add_student.university_rollno='".$roll_no."'order by admin_add_student.student_id asc";
}
else if($paper_type=="quiz")
{
    ...
    where results.coursename='".$course."' && results.sem_id=".$sem." && results.subject_code='".$sub."'&& results.quiz_marks!=0 && admin_add_student.university_rollno='".$roll_no."' order by admin_add_student.student_id asc";
}
else if($paper_type=="major")
{
    ...
    results.coursename='".$course."' && results.sem_id=".$sem." && results.subject_code='".$sub."' && results.major_marks!=0 && admin_add_student.university_rollno='".$roll_no."' order by admin_add_student.student_id asc";
}
else if($paper_type=="all")
{

    $flag=0;
    $s="Select student_id,university_rollno,name,photo from admin_add_student where coursename='".$course."' && sem_id=".$sem." &&university_rollno='".$roll_no."'order by admin_add_student.student_id asc";
    //echo $s."<br>";
